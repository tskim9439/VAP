...
## 요약
- 7개 DB 모두 실물 2대화씩으로 `dialogues.jsonl` 을 만들고, 6개(otoSpeech·AMI·NOTSOFAR-1·134-1·134-2·71631)는 Qwen3-ForcedAligner 로 화자 채널에서 실제 정렬해, 창 3개씩 시퀀스를 생성했다. ICSI 는 SPH(shorten) → FLAC 변환이 진행 중이라 변환된 회의 2개로 proxy 시각(발화 구간 균등 배치)으로 구조만 검증했다.
...
| ICSI (Bdb001·Bed002, FLAC) | 2회의 1.9 h | 6·6 | 2,319 | 78 (empty) | proxy | 143 / 19 | 3 | ALL_OK (proxy) |
...
3. **거의 무음인 창**(otoSpeech window-9: 30 s 에 90 ms 발화)이 생긴다. `min_text_tokens` 로 걸러야 한다.
4. **정렬 skip.** `--max-dur 60` 을 넘는 KO 긴 발화(71631 한 발화 35 s 이상도 있음)와 0.3 s 미만 발화는 정렬을 건너뛴다 → 그 발화는 미정렬로 창 제외. 71631 은 라벨 발화가 길어 max-dur 를 120 s 로 올릴지 검토.
...
- ICSI 는 proxy 시각이라 EOT 청크·활동의 실제 값은 정렬 뒤 다시 본다.
