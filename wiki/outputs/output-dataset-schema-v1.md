...
빌드 직후 `dataset.json` 카드가 자동 생성·검증된다(오류 0). 정렬은 2026-09-09 job 67076(71631·031/033)·67077(voxpopuli·yodas), 각 2 노드 96 shard, 모두 완료(prep-DONE).

| manifest | 정렬 스트림 | 빈 레코드 | 발화 | 토큰 | 소요 |
|---|---|---|---|---|---|
| aihub71631-train | 231,647 | 621 (0.3 %) | 231,026 | 2.38 M | 9 분 |
| aihub71631-dev | 66,365 | 677 (1.0 %) | 65,688 | 0.67 M | 6 분 |
| aihub-bc-train | 448,867 | 14 | 448,853 | 9.96 M | 21 분 |
| voxpopuli-train | 177,422 | 0 | 177,422 | 4.83 M | 18 분 |
| yodas-en129 | 133,853 | 513 (0.4 %) | 133,340 | 3.37 M | 9 분 |

71631 의 빈 레코드는 라벨 종료 시각이 wav 끝을 넘는 마지막 발화(122 파일)와 잘린 wav 1 개 — 리더가 파일 끝에서 잘라 읽도록 고쳤으므로(커밋 0f621e4 이후) 재정렬하면 대부분 채워진다.
카드 생성 단계는 67077 에서 실패했다: 컨테이너(root)가 만든 manifest 디렉토리에 SLURM 사용자(sa_tskim)가 쓸 수 없어서. 소유자를 바꾸고 카드를 다시 만들었으며, `ds_cards.py` 는 이 실패를 경고로 격하했다.
