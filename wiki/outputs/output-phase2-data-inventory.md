...
## 6b. 전량 빌드 정정 (2026-09-15, SLURM 70960)

134-1 실외 조각은 파일 수 기준으로 "1,492 대화 완전"이라 적었으나, 대화별로 라벨 발화 수와 조각 수를 대조하면 끝부분 조각이 빠진 대화가 많다: 조각 결손 46,020/426,780 발화(10.8 %), **완전 대화 459 (77.1 h)**, ≥95 % 368 (63.0 h). 134-2 실외는 §6 과 같다(완전 523, 90.1 h). 전량 통계는 [[task-phase2-data-prep]] 표.

## 6. 청소년 134-2 조각 검증 (2026-09-15)

A등급 편입의 두 조건을 서버에서 실측했다. 원자료 `raw/sources/experiments/2026-09-15-phase2-youth-crop-origin/`.

**(1) 채널 기원 — 통과.** 같은 대화에서 서로 다른 화자의 시간 겹침 조각 쌍(겹침 ≥0.3 s)을 라벨 시각으로 정렬해 겹침 구간의 최대 |상호상관| 을 쟀다(±40 ms lag 탐색). 혼합 마이크에서 잘렸다면 1 에 가깝고 각자 채널이면 0 에 가깝다.

| 집합 | 대화 / 쌍 | 중앙값 \|r\| | p90 | \|r\|>0.6 |
|---|---:|---:|---:|---:|
| 성인 134-1 실외(대조군, 채널 기원 확인됨) | 147 / 401 | 0.050 | 0.119 | 0 % |
| 성인 조각을 인위로 섞은 양성 대조 | 401 | 0.776 | 0.994 | 68 % |
| 청소년 134-2 실외 | 138 / 401 | 0.058 | 0.131 | 0 % |
| 청소년 134-2 실내 | 138 / 401 | 0.058 | 0.140 | 0 % |

청소년 조각은 성인 채널 기원 대조군과 같은 분포이고 인위 혼합과는 전혀 다르다. **청소년 조각도 화자별 채널에서 잘린 것**이다.

**(2) 대화별 완전성 — 실외만 부분 통과.** 라벨 발화 수 대비 보유 조각 수를 대화별로 셌다(라벨 FileLength 합산 시간).

| 집합 | 대화 | 라벨 시간 | 완전(100 %) | ≥95 % | ≥80 % | 일부 |
|---|---:|---:|---:|---:|---:|---:|
| 청소년 실외 | 1,649 | 399.9 h | 523 대화 90.1 h | 427 대화 75.4 h | 555 대화 170.2 h | 144 대화 64.2 h |
| 청소년 실내 | 7,828 | 2,021.7 h | 16 대화 0.2 h | 5 대화 0.1 h | 4,095 대화 782.8 h | 3,705 대화 1,238.6 h |

실내는 사실상 모든 대화에서 일부 발화가 빠져 있다(대화 단위 완전본 없음). 편입 규칙: 완전 대화(실외 90.1 h)는 성인 실외와 같은 A등급 자연 대화로 즉시 편입. ≥95 % 대화(실외 75.4 h)는 빠진 발화 주변 창을 turn 감독에서 마스크하고 편입. 실내 ≥80 % 대화(782.8 h)는 전사·활동 감독용으로만 쓰고, turn 감독은 연속 발화가 모두 있는 창에 한정한다(창 단위 유효 시간은 Q0 에서 산출). 결손 대화는 자연 gap·overlap·EOT 정답으로 쓰지 않는다.
...
- `raw/sources/experiments/2026-09-15-phase2-youth-crop-origin/` — 청소년 134-2 조각 채널 기원·완전성 실측(2026-09-15)
