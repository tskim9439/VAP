...
| NIA24 134-1 성인 발화 조각 | 화자 채널에서 라벨 시각대로 잘림(상호상관 r=1.000 검증) | 실외 1,492 대화(완전) ≈360 h; 실내 ≈7,700 대화(발화 45 %) | 라벨 StartTime 있음 → 복원 | 있음 | 자연 대화·turn(실외), 마스크 학습(실내) |
...
|---|---|---|---|
| KO | 71631 원본 248 h + 134-1 실외 ≈360 h = **≈610 h** | 186 복지 콜센터(발화 30,661), 132 연령대별 대화(≈수백 h 추정) | 134-1 실내 부분, 134-2 청소년(최대 수천 h), NIKL 준용 |
