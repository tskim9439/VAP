...
- D1 실학습 recipe(정본 §9 Q1, GPU 1 장): `sbatch --partition=apex --export=ALL,RUN=D1,MAX_TOKENS=28000,MIX=sqrt,EPOCHS=3,WARMUP=400,WORKERS=8 slurm/p2_train_d1.sbatch` — 7 코퍼스 동적 배치(예산 28 k 토큰, 창 ≤48, hop 10 s, 창 20–40 s, delays 2/3/4/6, R=6), sqrt 비중, lr 6e-5·adapter 1e-3, warmup 400, 3 epoch ≈ 12,190 step, EOT 가중 2·활동 가중 1·NEXT 0.3/0.15, 인코더 동결, 500 step 마다 저장·선점 시 requeue 재개. 예상 소요 ≈3 h(step 0.75 s + 로딩). 산출물 `/soundai/Model/VAPASR/p2-D1`. 이후: lane 상태 디코더(`vapasr/hf/lane_state.py`)·D1 평가.
