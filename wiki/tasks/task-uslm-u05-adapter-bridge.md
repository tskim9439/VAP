...
created: 2026-09-04
updated: 2026-09-04
summary: USLM U0.5 adapter bridge test — 4 run 완료(최선 oto 18.2–18.8 / 실내 17.5 / 실외 14.5–15.1 %), adapter 병목 아님; 관문 재정의 vs encoder unfreeze 판정 대기
sources:
...
- [x] **random-init 12k run 완료**(`u05-asr-noinit-12k`, lr 2e-4/5e-4, 65 min): step 3000 23.4/21.9/21.9 → 6000 23.4/21.7/22.8 → 9000 20.3/17.1/17.4 → **12000 20.1/17.7/16.6 %**. 9000→12000 에서 정체(lr 소진) → **이 레시피의 수렴점 ≈ 20/18/17 %**, 관문 미달. (같은 random-init 이라도 3000 step 짧은 run 이 18.7 % 로 더 좋았음 — 스케줄이 3000 에서 완전 감쇠했기 때문. 즉 이 문제는 step 수보다 **lr 감쇠 시점**에 민감.)
- [x] **증류-init lr 1e-4 12k run 완료**(`u05-asr-distill-12k`, 70 min): 2000 35.7/25.6/25.1 → 4000 22.6/20.8/20.7 → 6000 21.0/18.9/19.2 → **8000 18.8/17.5/14.5** → 10000 24.5/16.9/15.3 → **12000 19.3/17.8/15.1 %**. 8000 이후 정체·요동(val n=150, ±1–2 pt 잡음; 10000 의 24.5 는 스파이크).
- [x] **U0.5 종합 판정(4 run)**: 모든 레시피가 **oto 18–20 % / 실내 17–19 % / 실외 15–17 %** 에서 수렴. (a) 증류 init 의 이득은 잡음 범위(12k: 19.3/17.8/15.1 vs random 20.1/17.7/16.6) → **증류는 선택 사항, 필수 아님**(비용 20 min 이라 U1 에서는 유지). (b) 관문 원안(oto ≤16.0, KO ≤15.5) 은 실외만 통과(15.1), oto·실내 미달. (c) 그러나 같은 `[56,0]` 인코더의 자체 RNN-T(25.4/23.4/24.5) 대비 −6 pt 로 **adapter 병목은 아님** → 격차는 인과 인코더 상한 + 얼린 인코더. **결론: 연결 자체는 성립(backbone 유지). 관문 재정의 또는 인코더 상위 블록 unfreeze 1 회는 사용자 결정 대기.**
- [ ] **관문 정의 재검토(사용자 결정 필요)**: 현재 관문 "오프라인 Qwen ×1.15"(oto ≤16.0 %) 는 **비인과 오프라인 인코더**와 비교한다. 그러나 우리 인코더는 완전 인과 80 ms `[56,0]` 이며, 그 인코더의 **자체 RNN-T 디코더가 oto 25.4 / 실내 23.4 / 실외 24.5 %** 다. adapter+thinker 는 이미 18.2/19.2/16.1 % 로 **같은 인코더의 원래 디코더보다 낫다** → "adapter 가 정보를 잃는가" 라는 U0.5 의 원래 질문에는 이미 **아니오**. 남은 격차(오프라인 대비 +4–6 pt)는 adapter 가 아니라 **인과 인코더 자체의 상한**일 가능성이 크다(비교: `[56,13]` 1 s 미래 허용 시 RNN-T 19.0 %). 제안: 관문을 "Nemotron `[56,0]` RNN-T 보다 좋고, 오프라인 Qwen 대비 ≤ +50 % 상대" 로 재정의하거나, 상한 확인용으로 **Nemotron 인코더 상위 블록 unfreeze**(U1 에서 어차피 필요) 1 회를 추가.
