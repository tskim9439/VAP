...
- [x] **M 예산 (chunk 단위, 정렬 토큰 기준, δ=2, 테스트 80 발화)**: EN M=2 이월 2.8 % / M=3 0.9 %; KO M=2 **21.6 %** / M=3 6.1 % / M=4 2.6 % — 정렬기가 음절 토큰들에 같은 종료 시각을 주어 chunk 내 burst. → **M=4 기본(KO 2.6 %, EN 0.35 %)**, M=3 ablation. 전체 정렬 완료 후 재확인
- [~] Qwen3-ForcedAligner 정렬 — `experiments/u0_align.py` (aligner 단어/음절 시각 → BPE 토큰 종료 시각, offsets 매핑). 소규모 테스트 통과(발화당 0.33 s, 라벨 끝−마지막 토큰 끝 중앙값 EN 20 ms / KO 45 ms). **전체 실행 중** (otoSpeech 420 / AI Hub ts01-5 190 + vs02 186) → `$DATA_MANIFEST_DIR/align/`
