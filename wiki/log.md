...
- Changed: `decision-asr-backbone` → accepted(최종 조합, U0.5 관문, fallback), 신설 `task-uslm-u05-adapter-bridge`(p0, ~09-25), `task-uslm-u1-interleaved-asr` backbone 문구,
  `README.md`(§2 구조·§3 U0.5 행·관문·§5 TODO), `TODO.md`, `todo.md`, `index.md`, `status.md`.
- Reason: 사용자가 "Nemotron 3.5 FastConformer [56,0] → new adapter → Qwen3-ASR-0.6B-hf LM" 조합을 최종 선택. 근거: encoder 는 실측 causal ≤80 ms·
  잡음 강건·ko-KR, LM 은 오디오→텍스트 ASR 사전지식(thinker). 유일한 신규 위험(분포 간극)은 캐시된 Nemotron·AuT 특징(205 h)으로
  **표현 증류 초기화** 후 오프라인 ASR 미세조정으로 WER 관문(≤ +10–15 %)을 재는 U0.5 로 조기 판정. AuT+thinker 는 fallback.
- Next: U0(토큰율·정렬) 과 U0.5(bridge) 병행 착수. Stage 1 결과 페이지는 cc1s 재채점 후.
- By: tskim
