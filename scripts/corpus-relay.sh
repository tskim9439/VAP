...
set -uo pipefail
LOCAL=/Volumes/Samsung_T5/vapkt-corpora; REMOTE=/soundai/DB/raw; SSH_E="ssh -o ConnectTimeout=20 -o ServerAliveInterval=30 -p 3206"; SSH="$SSH_E mxc"
CA=~/.vapkt-ca-bundle.pem; LOG=$LOCAL/relay.log
log(){ echo "[$(date '+%F %T')] $*" | tee -a "$LOG"; }
ssh_retry(){ local i; for i in $(seq 1 40); do $SSH "$@" && return 0; log "ssh 실패, 60 s 후 재시도 ($i/40)"; sleep 60; done; return 1; }

upload(){ # upload <local dir> <remote dir>  — rsync 이어받기, 바이트 합계 검증
  local src="$1" dst="$2" i
  ssh_retry "mkdir -p '$dst'" || return 1
  for i in 1 2 3 4 5 6 7 8; do
    rsync -a --partial --exclude '*.part' --exclude 'run.log' --exclude '._*' --exclude '.DS_Store' -e "$SSH_E" "$src/" "mxc:$dst/" && break
    log "rsync 중단, 120 s 후 재개 ($i)"; sleep 120
  done
  local lb rb; lb=$(find "$src" -type f ! -name '*.part' ! -name run.log ! -name '._*' ! -name .DS_Store -exec stat -f %z {} + | awk '{s+=$1} END{print s+0}')
...
chunk_relay(){ # chunk_relay <url> <remote file> <chunk GB>: 로컬 디스크를 넘는 파일을 범위 다운로드 → 업로드 → 삭제로 릴레이, 서버에서 cat 으로 합침
  ...
  while [ "$off" -lt "$total" ]; do
    ...
    for i in 1 2 3 4 5; do rsync -a --partial -e "$SSH_E" "$tmp/$part" "mxc:$rparts/" && break; sleep 60; done
...
fetch_full(){ # fetch_full <url> <local file>: Content-Length 와 같아질 때까지 curl -C - 반복(최대 30 회)
  local url="$1" out="$2" want have i; want=$(curl -sIL --cacert $CA "$url" | grep -i '^content-length' | tail -n 1 | tr -dc '0-9'); [ -n "$want" ] || { log "!! 크기 확인 실패 $url"; return 1; }
  for i in $(seq 1 30); do have=$(stat -f %z "$out" 2>/dev/null || echo 0); [ "$have" = "$want" ] && return 0; [ "$have" -gt "$want" ] && rm -f "$out"; curl -sS -L --cacert $CA -C - -o "$out" "$url" 2>>"$LOG" || sleep 90; done
  [ "$(stat -f %z "$out" 2>/dev/null || echo 0)" = "$want" ] || { log "!! $(basename "$out") 크기 불일치 have=$(stat -f %z "$out" 2>/dev/null) want=$want"; return 1; }
}
stage_chime6(){ # CHiME-6 (OpenSLR 150, CC BY-SA 4.0): 작은 것은 통째로(완전성 검증), train(97 GB)은 chunk 릴레이
  mkdir -p "$LOCAL/chime6"; local B=https://www.openslr.org/resources/150 f okall=1
  for f in LICENSE.txt CHiME6_transcriptions.tar.gz CHiME6_floorplans.tar.gz CHiME6_dev.tar.gz CHiME6_eval.tar.gz; do fetch_full "$B/$f" "$LOCAL/chime6/$f" || okall=0; done
  [ "$okall" = 1 ] && finish chime6-small "$LOCAL/chime6" "$REMOTE/chime6" || log "!! chime6 소형 파일 미완성 — 업로드 보류(로컬 보존)"
  chunk_relay "$B/CHiME6_train.tar.gz" "$REMOTE/chime6/CHiME6_train.tar.gz" 10
}

case "${1:-}" in
  upload) upload "$2" "$3" ;;
  chime6) log "=== CHiME-6 단계만 실행 ==="; stage_chime6; log "=== CHiME-6 단계 종료 ===" ;;
  chime6-train) log "=== CHiME-6 train chunk 릴레이만 실행(소형 파일은 완료됨) ==="; until $SSH true 2>/dev/null; do log "mxc SSH 대기(포트 닫힘)"; sleep 300; done; chunk_relay https://www.openslr.org/resources/150/CHiME6_train.tar.gz "$REMOTE/chime6/CHiME6_train.tar.gz" 10 && log "CHiME-6 train 완료" || log "!! CHiME-6 train 실패"; log "=== CHiME-6 train 종료 ===" ;;
  run)
    log "=== 릴레이 시작 ==="
    ...
    if ls -lO "$HOME/Desktop/VAPKT/.env.local" 2>/dev/null | grep -q dataless; then log "!! .env.local 이 iCloud 에 퇴출(dataless)되어 읽을 수 없음 — NOTSOFAR-1 단계 건너뜀(나중에 재실행)"; else set -a; source "$HOME/Desktop/VAPKT/.env.local"; set +a; fi
    [ -n "${HF_TOKEN:-}" ] && for sub in benchmark-datasets/dev_set/240825.1_dev1 benchmark-datasets/train_set/240825.1_train benchmark-datasets/eval_set/240825.1_eval_full_with_GT; do
