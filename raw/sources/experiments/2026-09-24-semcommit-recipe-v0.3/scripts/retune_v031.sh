#!/usr/bin/env bash
# v0.3.1: same v0.3 teacher rows, thresholds re-tuned with a per-branch precision floor (punct / other), regrade all sets, score vs gold. CPU only.
set -uo pipefail
cd /home/tskim/VAP
...
L3=/data4/tskim/semcommit/labels/v0.3; L31=/data4/tskim/semcommit/labels/v0.3.1; mkdir -p $L31
...
for s in $GS; do $GT score-labels --words ${W[$s]} --labels $L31/labels-$s.jsonl --gold $GD/gold-$s.jsonl > $GD/scores/teacher-v0.3.1-$s.json; echo "[v0.3.1 labels vs gold] $s $(cat $GD/scores/teacher-v0.3.1-$s.json)"; done
echo "[$(date "+%F %T")] LABELS_V031_DONE"
