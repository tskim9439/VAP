#!/usr/bin/env bash
# r4: same settings as r3 but v0.3.2 labels (per-branch floor → "other" branch off in both languages; Korean reply-word units before more speech → B). Waits for the v0.3 chain (r3 eval) to free GPU 0.
set -uo pipefail
cd /home/tskim/VAP
...
GD=/data4/tskim/semcommit/gold/v1; V0=/data4/tskim/semcommit/data/v0; L31=/data4/tskim/semcommit/labels/v0.3.2
R=/data4/tskim/semcommit/runs/v0.3.2-r4; O=/data4/tskim/semcommit/eval/v0.3.2-r4
...
echo "[$(date "+%F %T")] waiting for the v0.3 chain"
...
for s in ls-test gs-test ks-eval ks-long; do
  step $P experiments/semcommit_eval.py --model $R/final --encoder $NEMO --words ${W[$s]} --labels $L31/labels-$s.jsonl --delay 4 --sem-bias -2 -1 0 1 2 --theta 0.2 0.35 0.5 --gpu 0 --batch-size 32 --out $O/r4-d4-$s.json
  $GT score-eval --words ${W[$s]} --gold $GD/gold-$s.jsonl --streams $O/r4-d4-$s.streams.jsonl --out $GD/scores/models-r4-$s.json
done
echo "[$(date "+%F %T")] R4_DONE"
