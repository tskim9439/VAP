#!/usr/bin/env bash
# v0.3.2: v0.3.1 + reply-word branch (punct_resp: a punctuated unit of only reply words before more speech), tuned per branch like v0.3.1. CPU only.
set -uo pipefail
cd /home/tskim/VAP
...
L3=/data4/tskim/semcommit/labels/v0.3; L31=/data4/tskim/semcommit/labels/v0.3.2; mkdir -p $L31
...
for s in $GS; do $GT score-labels --words ${W[$s]} --labels $L31/labels-$s.jsonl --gold $GD/gold-$s.jsonl > $GD/scores/teacher-v0.3.2-$s.json; echo "[v0.3.2 labels vs gold] $s $(cat $GD/scores/teacher-v0.3.2-$s.json)"; done
echo "[$(date "+%F %T")] LABELS_V032_DONE"
