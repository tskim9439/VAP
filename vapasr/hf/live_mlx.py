...
class MlxThinker:
    def __init__(self, path: str):
        self.model, self.tok = mlx_load(path); self.emb_w = self.model.model.embed_tokens.weight     # (V, D)
        self.cache = None; self.reset()
        mx.eval(self.emb_w)
    def reset(self): self.cache = make_prompt_cache(self.model)
    def embed_ids(self, ids) -> mx.array: return self.emb_w[mx.array(list(ids))]                # (T, D)
    def embed_id(self, tid: int) -> mx.array: return self.emb_w[tid]
    def step(self, e: mx.array) -> mx.array:
        """e (T, D) 또는 (D,) → 마지막 위치 logits (V,) float32"""
        if e.ndim == 1: e = e[None]
        logits = self.model(mx.zeros((1, e.shape[0]), dtype=mx.int32), cache=self.cache, input_embeddings=e[None].astype(self.emb_w.dtype))
