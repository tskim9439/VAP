...
def iter_71631(label_root: str, audio_root: str, subsets: Tuple[str, ...] = ("TL_01.실내",), audio_subsets: Tuple[str, ...] = ("TS_01.실내_5", "VS_02.실외"), vad_hop_ms: float = 20.0) -> Iterator[dict]:
    ...
    for sub in audio_subsets:
        for p in glob.glob(os.path.join(audio_root, "*", "*", sub, "*.wav")): wavs[os.path.splitext(os.path.basename(p))[0]] = p
    for sub in subsets:
        for jp in sorted(glob.glob(os.path.join(label_root, sub, "**", "*.json"), recursive=True)):
...
def iter_bc(roots: List[str], split: str = "Training") -> Iterator[dict]:
    ...
    for stem, (az, member, nbytes) in audio.items():
        ...
        genre = re.search(r"_([^_]+)\.zip$", os.path.basename(az)).group(1); pair = os.path.basename(az).split("_")[1]
