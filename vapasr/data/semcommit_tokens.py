"""Semantic commit 토큰 registry — <SEM_END>(의미 단위 확정)·<TURN_END>(v0: 발화 종료 proxy) (raw/inbox/streaming_asr_semantic_commit_plan.md §2·§13).

id 정책: Phase 1(interleave_data.SPECIAL_TOKENS, 151705–151716) → Phase 2(dialogue_tokens.PHASE2_SPECIALS, 151717–151722) → SEM(151723–151724) 순서로
...
mono(lanes=0) 모델에서 Phase 2 id 는 '예약'만 된다(임베딩 행은 평균+잡음, decode 차단, config.sem_reserved 에 기록 → 나중 add_phase2_tokens 가 재초기화). 임베딩 여유 행(151936 − 151725 = 211)이 있어 resize 는 필요 없다.
...
SEM_REGISTRY = {"<SEM_END>": 151723, "<TURN_END>": 151724}     # Qwen3-ASR-0.6B tokenizer(기본 길이 151705) 기준 — FROZEN_REGISTRY 바로 뒤
...
def sem_ids_of(ids: Dict[str, int]) -> Tuple[int, int]:
    """{name: id}(sp_ids 또는 sem_registry) → (<SEM_END> id, <TURN_END> id)."""
    return int(ids["<SEM_END>"]), int(ids["<TURN_END>"])
...
def semantic_blocked_ids(blocked_ids: Iterable[int], ids: Dict[str, int], lanes: int, block_reserved_phase2: bool = True) -> List[int]:
    """decode 차단 목록 갱신: SEM/TURN 은 항상 방출 가능(차단 해제), mono(lanes=0) 이면 학습되지 않은 Phase 2 예약 행(<SPK_3..6>,<ONSET>,<EOT>)을 차단."""
    out = set(int(b) for b in blocked_ids) - {ids[n] for n in SEM_SPECIALS if n in ids}
    if block_reserved_phase2 and lanes == 0: out |= {ids[n] for n in PHASE2_SPECIALS if n in ids}
    return sorted(out)
