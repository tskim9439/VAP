#!/usr/bin/env bash
# 로컬 Mac 에서 실시간 음성인식 서버 실행:  bash experiments/live/run_local.sh [--delay 4] [--port 8765]
...
echo "model=$MODEL device=$DEV"; exec python -u experiments/live/server.py --model "$MODEL" --device "$DEV" --default-delay "${DELAY:-4}" --port "${PORT:-8765}" "$@"
