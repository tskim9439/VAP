#!/usr/bin/env bash
# 최종 보고용 test 평가(선정된 final 하나): test-clean · test-other · kspon-eval · ah71-dev(71631 은 test 분할이 없어 held-out dev) 를 δ=2/4 로 전체 평가.
...
SETS="test-clean=librispeech-test:test-clean:stream,test-other=librispeech-test:test-other:stream,kspon-eval-clean=kspon-eval:eval_clean:utt,kspon-eval-other=kspon-eval:eval_other:utt,ah71-dev=aihub71631-dev:dev:utt"
for d in 2 4; do
