...
set -uo pipefail
R=${R:-/soundai/Model/VAPASR/hf-D2}; C2=${C2:-/soundai/Model/VAPASR/hf-C2}; S=$R/eval-ckpts; GPU=${CUDA_VISIBLE_DEVICES:-4}; EXTRA="--dev-extra ah71-dev=aihub71631-dev:dev:utt --num-workers 0"
while [ ! -f $R/DONE ]; do sleep 60; done; echo "[$(date '+%F %T')] DONE 확인"
run() { local init=$1 out=$2 tag=$3 d=$4 b=$5 ns=$6 nu=$7; echo "[$(date '+%F %T')] $tag ← $init (δ=$d bias=$b)"; CUDA_VISIBLE_DEVICES=$GPU python experiments/s3_train_hf.py --eval-only --init "$init" --out-dir "$out" --sentinel-stream $ns --sentinel-utt $nu --eval-seed 7 --eval-tag "$tag" --eval-delay $d --eval-bias $b $EXTRA 2>&1 | grep -E "_score|Traceback|Error" | cut -c1-700; }
