...
ap.add_argument("--no-check-save", action="store_true", help="학습 뒤 final 재로드 parity 검사(손실·state_dict 대조)를 끈다 — 기본은 켬(2026-09-17 인코더 미저장 사고 재발 방지)")
...
if not a.train_encoder:
    ...
    for p_ in model.encoder.parameters(): p_.requires_grad_(True)
    model.config.encoder_trainable = True; model.encoder.train(); log(f"인코더 학습(lr {a.lr_encoder}) · 체크포인트에 인코더 저장")
...
if done and not a.no_check_save and main:
    ...
    l1, l2 = batch_loss(mem, xb), batch_loss(re, xb); parity = (not miss and not extra and not shape and not diff and not ediff and abs(l1["loss_text"] - l2["loss_text"]) <= 0.05 * max(l1["loss_text"], 0.05))
    json.dump(dict(parity=parity, memory=l1, reloaded=l2, missing=miss, extra=extra, shape_mismatch=shape, value_diff=diff[:50], encoder_diff=ediff[:50]), open(os.path.join(out, "parity.json"), "w"), indent=1)
    log(("check-save PARITY OK" if parity else "!! check-save PARITY FAIL — 저장된 final 이 학습 모델과 다르다(위 차이 목록·parity.json 확인)"))
    del re; torch.cuda.empty_cache()
if done:
